...
Note that if you use IntelliJ, preconfigured run configurations of the explained tasks on this page are available in the **.run** directory in the project root.

## Building locally

### Development build

The basic development build, which you most of the time run locally in your IDE, will get you a basic running application. You are, however, missing stuff like codesigning, AOT training caches, and proper versioning. You can run the application in local development mode with `gradlew run`.

To customize the application behavior during local development, the build script will automatically generate the file **app/dev.properties**. This file is designated to pass custom properties to your application in development mode. This way you can adjust the application behavior when being run while developing versus actual releases.
...
When configured and enabled, the production build workflow can include all steps like signing the code, training AOT data, and more. To properly run this build, you will need to have all additional required tools installed for installer generation, signing, and more. For instructions on how to configure the environment to enable signing, see the [Windows](windows-signing) and [macOS](/notarization) pages.

#### AOT cache

Steps like training the [AOT cache](/aot) are only done in the CI/CD pipeline by default, but can be enabled by changing the variable *enableAot* in the root **build.gradle**.

#### Debugging

The generated production applications can be debugged via the automatically generated debug scripts. You can find them in the **scripts** directory of the generated application images. The debug scripts launch the JVM with verbose logging options and other properties. You can customize which properties are passed and how the scripts look like in the **dist/debug** directory.

To automatically attach the debugger on production app startup where the app will wait idle until it is attached, it is recommended to use [AttachMe](https://github.com/JetBrains/attachme) in IntelliJ. Then, passing these additional JVM options in the debug script will make your app automatically attach:

```
-javaagent:<home dir>/.attachme/attachme-agent-1.2.11.jar=port:7857,host:localhost
-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=127.0.0.1:0
-XX:+EnableDynamicAgentLoading
```

If you don't want to use AttachMe, you can also just use the agentlib option alone. If you do that, you will have to attach you debugger manually each time though.
