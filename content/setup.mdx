...
This documentation is not designed to be a handholding experience. It is expected that you already have experience in JavaFX application development. Many areas are not explicitly covered here, so it is also expected that you read through the source code yourself. Getting a feel for the codebase structure and source code layout is important if you actually want to commit using it for your application.
...
Once the project builds successfully, you can look into overriding the application logos with your own logo on the [logos page](/logo).
